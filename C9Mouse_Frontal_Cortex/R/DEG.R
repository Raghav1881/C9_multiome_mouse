...
runMAST <- function(seu_obj, idents = NULL) {
  markers_C9HET <- list()
  markers_C9KO <- list()
  ...
  for (l in ident_list) {
    tryCatch({
      ...
      cat(paste("Error in processing C9HET for ident", l, ":", conditionMessage(e), "\n"))
    })
    tryCatch({
      markers_C9KO[[l]] <- find_markers_RNA(seu_rna[[l]], 'C9KO')
    }, error = function(e) {
      cat(paste("Error in processing C9KO for ident", l, ":", conditionMessage(e), "\n"))
    })
  }
...
runDEG <- function(hip.combined, idents = NULL, selected_clusters = NULL) {
  Idents(hip.combined) <- idents
  sub <- subset(hip.combined, idents = selected_clusters) %>%
    droplevels()
  out <- runMAST(sub, idents)
  return(out)
}
