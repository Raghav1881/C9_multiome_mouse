...
load_data <- function(file_dir) {
  ...
  for (k in seq_along(fragments)) {
    cnames <- c(cnames, toupper(strsplit(strsplit(counts[k], '/')[[1]][1], '_')[[1]][4]))
    mnames <- c(mnames, toupper(strsplit(strsplit(metadata[k], '/')[[1]][1], '_')[[1]][4]))
    fnames <- c(fnames, toupper(strsplit(strsplit(fragments[k], '/')[[1]][1], '_')[[1]][4]))
    pnames <- c(pnames, toupper(strsplit(strsplit(peaks[k], '/')[[1]][1], '_')[[1]][4]))
  }
...
prep_data <- function(file.list, file_dir) {
  ...
  for (sample in file.list) {
    n <- n + 1
    # Load counts
    counts <- Read10X_h5(file.path(file_dir, sample$counts))
  ...
  combined.peaks <- reduce(x = c(seurat.list$HET2$peaks, seurat.list$HET3$peaks, 
                                 seurat.list$HET5$peaks, seurat.list$HET6$peaks,
                                 seurat.list$HET7$peaks, seurat.list$KO1$peaks, 
                                 seurat.list$KO2$peaks, seurat.list$KO3$peaks, 
                                 seurat.list$KO4$peaks, seurat.list$WT2$peaks, 
                                 seurat.list$WT3$peaks, seurat.list$WT4$peaks, 
                                 seurat.list$WT5$peaks, seurat.list$WT6$peaks))
  peakwidths <- width(combined.peaks)
  combined.peaks <- combined.peaks[peakwidths < 10000 & peakwidths > 20]
  n <- 0
  cnames <- names(seurat.list)
  for (sample in seurat.list) {
    ...
    seurat.list[[cnames[n]]] <- seu.obj[, colnames(sample$counts[["Peaks"]])]
    seurat.list[[cnames[n]]][['RNA']] <- CreateAssayObject(sample$counts$`Gene Expression`)
  }
...
add_metadata <- function(seurat.list) {
  ...
  new.list[[13]]$sex <- "Female"
  new.list[[14]]$sex <- "Female"
  return(new.list)
}

# Integration RNA
integrateRNA <- function(merged, num_pcs = 50, resolution = 1.6, k_anchor = 25, ref = c(1,2,3,4,5)) {
...
# Integration peaks
integratePeaks <- function(seu.obj, ident = 'subclass', ref= c(1,2,3,4,5)) {
  ...
  peaks.list <- readRDS('r_objs/ATAC/peaks_list.rds')
  ...
  merged <- readRDS('r_objs/annotated.rds')
  peaks.combined <- readRDS('r_objs/peaks_combined.rds')
  ...
  merged[["peaks"]] <- peaks.combined[['peaks']]
  ...
  merged$peaks_clusters <- Idents(peaks.integrated)
  Idents(merged) <- 'peaks_clusters'
